...
import { OfertaBadge, OfertaPrice } from '../components/OfertaBadge';
import { getProductImage, tieneOfertaVigente } from '../utils/product.utils';
  ...
  return (
    <div className="min-h-screen bg-[#F5F2EB]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link to="/catalogo" className="flex items-center gap-2 text-[#5F6368] hover:text-[#D97706] mb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="mb-2 flex flex-wrap items-center gap-3">
                <p className="text-[#5F6368]">{product.categoria?.nombre ?? 'Sin categoría'}</p>
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="flex items-center border border-[#E5E2DA] rounded-xl overflow-hidden">
                  <button
                    type="button"
                  ...
                  <button
                    type="button"
                    onClick={() => setQuantity(Math.min(product.stockDisponible, quantity + 1))}
                    className="p-3"
                ...
                <span className="text-sm text-[#5F6368]">Disponibles: {product.stockDisponible}</span>
              </div>
              <div className="flex gap-4">
                <button
                  type="button"
                  onClick={() => addToCart(product, quantity)}
                  className="flex-1 bg-[#D97706] text-white py-4 rounded-xl font-semibold"
                >
                  Agregar al carrito
                </button>
                <button
                  ...
                  className="border-2 border-[#D97706] text-[#D97706] px-6 py-4 rounded-xl font-semibold"
                >
                  Comprar ahora
                </button>
              </div>
        ...
        {relatedProducts.length > 0 && (
          <section className="mt-16">
            <h2 className="text-2xl font-bold text-[#2B2D31] mb-6">Productos Relacionados</h2>
